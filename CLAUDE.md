...
8. `prepend_to_archive()` updates `news2.html`, the running monthly archive:
   ...
   - Also opportunistically fixes W3C-invalid `<meta>` tags, injects a responsive-image CSS rule into the archive's `<head>` if missing, strips obsolete `<font>` tags and invalid `img width="100%"` attributes from the archive's own static chrome, and moves that same chrome's legacy `width`/`valign`/`align`/`cellspacing`/`cellpadding`/`border` table attributes to CSS (`normalize_head_meta()`, `ensure_responsive_images()`, `fix_legacy_font_tags()`, `fix_legacy_img_width()`, `fix_static_chrome_table_attrs()`, `fix_year_index_tables()`). These deliberately skip the shared `h_news()`-generated entry-card tables (identified by `_SHARED_ENTRY_TABLE_SIGNATURES`), which must stay on legacy presentational attributes since that markup is also used for `news4.html`, and email clients need them (see step 4 above) — `news2.html` itself is never emailed, so its *own* chrome (masthead/nav/footer/year-index tables) has no such constraint. All of these run on every invocation and the file is written whenever any of them changed something, even if there's no new entry to append that run.
...
`Prune Month/` is a companion workflow, separate from the daily conversion pipeline above: at the end of each month (or the start of the next, before that month's first `tribulation_times_convert.py` run) the site owner runs `prune_month.py` from inside this folder to remove the completed month's daily entry cards from `news2.html` and add a link to the corresponding old-format monthly archive (e.g. `sept26.html`) in the year/month table at the bottom of the page.

- `prune_month.py` reads/writes its own `news2.html` copy inside this folder — a staging copy, kept separate from the one at the repo root that `tribulation_times_convert.py` maintains day to day.
- `template.html` and the Ladder-quote source archives (e.g. `oct08.html`, `nov08.html`) are dependencies for `prune_month.py --refresh-template`; `Files necessary to run monthly prune.txt` documents what's needed for a given month's run.
- `prune_month.bat` is the double-click entry point, mirroring `ConvertUpdate.bat` for the main converter.
- **This folder is tracked in git, not a scratch workspace** — per the Git workflow rule above, commit and push any changes made here (the script itself or its dependency files) the same as changes anywhere else in the repo.
- `add_archive_link()` (in `prune_month.py`) detects vertical-alignment on the year-index table's empty placeholder cells from either the legacy `valign="top"` HTML attribute or its CSS equivalent `style="vertical-align:top;"`, and always writes the modern CSS form back — whichever form is present in a given `news2.html` depends on whether `tribulation_times_convert.py`'s chrome cleanup (see step 8 above) has already run on it. Keep both forms working if you ever touch that detection logic.
