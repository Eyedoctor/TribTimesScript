...
def get_following_features(link, soup, doc_order=None):
    ...
    for idx_list, list_tag in enumerate(owned_lists):
        ...
        for li in list_tag.find_all("li"):
            lt = " ".join(li.get_text(" ", strip=True).split())
            if lt:
                feature_items.append(lt)
...
def get_following_excerpts(link, _ladder_text="", _current_url=""):
    """
                    ...
                    def walk_until_list(start):
                        ...
                        for _ in range(80):
                            ...
                            if isinstance(n, NavigableString):
                                ...
                                if n.name in ("ul", "ol"):
                                    # Collect bullet text and continue (don't stop)
...
def h_news(source, url, link_text, excerpts=None, feature_items=None):
    ...
    if feature_items and len(all_excerpts) > 1 and len(all_excerpts) <= 6:
        ...
        # Long-form summary OR no feature list — inline everything as paragraphs
        if feature_items and len(all_excerpts) > 6:
            # Convert feature_items to inline excerpts so all content flows together
            for fi in (feature_items or []):
                if fi.startswith("__LABEL__"):
                    lbl = fi[len("__LABEL__"):]
                    all_excerpts.append(
                        f'<strong style="font-family:Verdana,Arial,sans-serif;'
                        f'font-size:10px;letter-spacing:2px;text-transform:uppercase;'
                        f'color:#7a1c1c;">{lbl}</strong>')
                elif not fi.startswith("__POST_LIST__"):
                    all_excerpts.append(fi)
            feature_items = []
    ...
    for para in pre_excerpts:
        if para.startswith("<strong") and para.endswith("</strong>"):
            ...
            ex_html += (
                ...
                f'color:{C["ink_mid"]};line-height:1.7;margin:6px 0 0 0;">{para}</p>'
            )
...
def build_entry_html(entry, include_header=True):
    ...
    if collect_text:
        out += f"""<div style="background-color:#f0e8e8;border-left:3px solid {C['crimson']};
...
color:{C['ink_mid']};margin:0;line-height:1.6;">{sr_text}</p>
    ...
    # Document-order position of the social post, so it lands in the right place.
    def _src_pos(u):
        if not u:
            return 10 ** 9
        p = block.find(u)
    ...
    sr_pos = _src_pos(sr_url) if sr_text else -1
    sr_done = not bool(sr_text)

    # News items
    for item in get_news_items(block):
        # Insert the social post before the first news item that follows it
        # in the source.
        if not sr_done and _src_pos(item.get("url", "")) > sr_pos:
            out += sr_html
            sr_done = True
    ...
    # X post came after every news item (or there were none).
    if not sr_done:
        out += sr_html
