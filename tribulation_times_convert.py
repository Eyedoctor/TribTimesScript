...
# Style signatures identifying <table> tags generated by the SHARED
# h_news()/get_social_report()/repair_entry() code that also renders
# news4.html's entry cards -- fix_static_chrome_table_attrs() must never
# touch these, since news2.html is the only page free of the email
# rendering constraint that keeps this markup on legacy attributes.
_SHARED_ENTRY_TABLE_SIGNATURES = (
    "margin-bottom:18px;padding-bottom:18px;border-bottom:1px dashed",
    'cellspacing="0" cellpadding="4"',   # feature/bullet list inner table
    "border-collapse:collapse",          # year-index tables (handled below)
)


def fix_static_chrome_table_attrs(html):
    """Move width/valign/align/cellspacing/cellpadding/border="0"
    presentational attributes on <table>/<td> tags in news2.html's own
    static masthead/nav/footer chrome into inline CSS. This markup is
    unique to the archive page (never shared with news4.html's
    email-facing templates -- see _SHARED_ENTRY_TABLE_SIGNATURES, which
    this skips), so unlike the entry cards it can move fully off legacy
    attributes with no email-compatibility tradeoff.
    Returns (possibly modified html, True/False whether anything changed).
    """
    def _fix_tag(m):
        tag = m.group(0)
        if any(sig in tag for sig in _SHARED_ENTRY_TABLE_SIGNATURES):
            return tag
        style_extra = []
        for attr, css in (("valign", "vertical-align"), ("align", "text-align")):
            am = re.search(rf'\s{attr}="([^"]*)"', tag)
            if am:
                style_extra.append(f"{css}:{am.group(1)};")
                tag = tag[:am.start()] + tag[am.end():]
        wm = re.search(r'\swidth="([^"]*)"', tag)
        if wm:
            val = wm.group(1)
            style_extra.append(f"width:{val if val.endswith('%') else val + 'px'};")
            tag = tag[:wm.start()] + tag[wm.end():]
        for attr in ("cellspacing", "cellpadding", "border"):
            tag = re.sub(rf'\s{attr}="[^"]*"', '', tag)
        if style_extra:
            extra = "".join(style_extra)
            if 'style="' in tag:
                tag = tag.replace('style="', f'style="{extra}', 1)
            else:
                tag = re.sub(r'>\s*$', f' style="{extra}">', tag)
        return tag
    fixed = re.sub(r'<(?:table|td)\b[^>]*>', _fix_tag, html)
    return fixed, (fixed != html)


def fix_year_index_tables(html):
    """The two 'browse by year' index tables in news2.html's footer rely
    on border="1"/cellpadding="4" for their visible per-cell grid lines
    -- a plain style="" on the <table> itself can't replicate that (a
    table[border] attribute implicitly borders every cell, which CSS has
    no single-declaration equivalent for), so this adds a shared class
    with one CSS rule instead and strips the legacy attributes. Their
    cellspacing="4" is already inert (the tables' own inline style sets
    border-collapse:collapse, under which CSS ignores cellspacing), so
    it's simply dropped with no visual change.
    Returns (possibly modified html, True/False whether anything changed).
    """
    changed = False
    def _fix(m):
        nonlocal changed
        tag = m.group(0)
        if "border-collapse:collapse" not in tag or 'cellspacing="4"' not in tag:
            return tag
        tag = re.sub(r'\s+cellspacing="4"', '', tag)
        tag = re.sub(r'\s+cellpadding="4"', '', tag)
        tag = re.sub(r'\s+border="1"', '', tag)
        tag = re.sub(r'^<table', '<table class="year-index-table"', tag, count=1)
        changed = True
        return tag
    fixed = re.sub(r'<table\b[^>]*>', _fix, html)
    if changed and '.year-index-table td' not in fixed:
        rule = ("\n.year-index-table{border-collapse:collapse;}\n"
                ".year-index-table td{border:1px solid #c8b080;padding:4px;}\n")
        m = re.search(r'</style>', fixed, flags=re.IGNORECASE)
        if m:
            fixed = fixed[:m.start()] + rule + fixed[m.start():]
...
def prepend_to_archive(archive_path, entries):
    ...
    html, year_table_fixed = fix_year_index_tables(html)
    if year_table_fixed:
        print("  Moved year-index table borders/padding to CSS.")

    html, chrome_attrs_fixed = fix_static_chrome_table_attrs(html)
    if chrome_attrs_fixed:
        print("  Moved obsolete table/td attributes in archive chrome to CSS.")
    ...
    if not fresh:
        ...
        if (head_fixed or css_fixed or font_fixed or img_width_fixed
                or year_table_fixed or chrome_attrs_fixed):
